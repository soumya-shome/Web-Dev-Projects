...
<?php 
$r1=mysqli_query($con,"SELECT `Exam Code` FROM `result` WHERE `Student Id`='".$_SESSION['id']."'");
$row=mysqli_fetch_array($r1);
$r2=mysqli_query($con,"SELECT `Course Id` FROM `exam` WHERE `Exam Code`='".$row[0]."'");
$row1=mysqli_fetch_array($r2);
	$s="INSERT INTO `exam`(`Exam Code`, `Exam Date`, `Exam Slot`, `Machine No.`, `Student Id`, `Course Id`, `Paper Code`, `Activation No.`, `Exam Status`) VALUES ('".$_SESSION['eId']."','','','','".$_SESSION['id']."','".$row1[0]."','','','Applied')";
					mysqli_query($con,$s);
echo "ReApplied";
